...
function load_dataset() {
    $filename = "data.csv"; // link to the dataset
...
function standardize_data($dataset) {
    ...
    // Standardize dataset with rating scale adjustments where: 1 = 0%, 2 = 50% , 3 = 100%
    foreach ($dataset as &$row) {
        for ($i = 0; $i < 36; $i++) {
            if ($std_devs[$i] != 0) {
                ...
                if ($row[$i] == 1) {
                    $row[$i] = $standardized_value * 0; // Scale for rating 1
                } elseif ($row[$i] == 2) {
                    $row[$i] = $standardized_value * 0.5; // Scale for rating 2
                } elseif ($row[$i] == 3) {
                    $row[$i] = $standardized_value * 1; // Scale for rating 3
                } else {
